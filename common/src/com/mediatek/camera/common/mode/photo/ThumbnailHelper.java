...
public class ThumbnailHelper {
    ...
    public static final String CAPTURE_TYPE_THUMBNAIL = "thumbnail";
    public static final String FORMAT_TAG = CAPTURE_TYPE_THUMBNAIL;
    ...
    public static byte[] getDataFromImage(Image image) {
        ...
        if (planes == null || planes.length <= 0){
            LogHelper.w(TAG, "[getDataFromImage] Fail to get planes");
            return null;
        }
        ...
        LogHelper.v(TAG, "get data from " + planes.length + " planes");
        for (int i = 0; i < planes.length; i++) {
            int shift = (i == 0) ? 0 : 1;
            buffer = planes[i].getBuffer();
            if (buffer == null){
                LogHelper.w(TAG, "[getDataFromImage] Fail to get byte buffer from plane");
                return null;
            }
            rowStride = planes[i].getRowStride();
            pixelStride = planes[i].getPixelStride();
            if (pixelStride>0){
                LogHelper.d(TAG, "[getDataFromImage] pixel stride " + pixelStride + " is invalid");
            }
            LogHelper.v(TAG, "pixelStride " + pixelStride);
            LogHelper.v(TAG, "rowStride " + rowStride);
            LogHelper.v(TAG, "width " + width);
            LogHelper.v(TAG, "height " + height);
            // For multi-planar yuv images, assuming yuv420 with 2x2 chroma subsampling.
            int w = crop.width() >> shift;
            int h = crop.height() >> shift;
            buffer.position(rowStride * (crop.top >> shift) + pixelStride * (crop.left >> shift));
            if (rowStride >= w){
                LogHelper.d(TAG, "[getDataFromImage] rowStride " + rowStride + " should be >= width " + w);
            }
            ...
            LogHelper.v(TAG, "Finished reading data from plane " + i);
        }
        return data;
    }
    ...
    private static void checkAndroidImageFormat(Image image) {
        ...
        switch (format) {
            ...
            case ImageFormat.YV12:
                if (3 != planes.length){
                    throw new IllegalArgumentException("YUV420 format Images should have 3 planes");
                }
                break;
            default:

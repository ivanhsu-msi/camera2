...
public class NormalRecorder implements IRecorder {
    ...
    public void init(RecorderSpec spec) {
        ...
        if (spec.isRecordAudio) {
            ...
            if (spec.isHEVC) {
                spec.profile.videoCodec=MediaRecorder.VideoEncoder.HEVC;
            }
            ...
            if (spec.isHEVC) {
                ...
                LogHelper.d(TAG, "spec.profile.videoCodec"+spec.profile.videoCodec);
            }
        }
        if(spec.hdr10Support){
            mMediaRecorder.setHdr10(1);
        }
    ...
    private void checkRecorderSpec(RecorderSpec spec) {
        if (spec.profile == null) {
            LogHelper.e(TAG, "profile = " + spec.profile );

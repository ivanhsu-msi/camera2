...
public class VideoDevice2Controller extends Device2Controller
    ...
    private static final int HANDLER_TAKE_VSS_CAPTURE = 2;
            ...
            if (mSession != null) {
                synchronized (mSession) {
                    if (mSession != null) {
                        ...
                        LogHelper.e(TAG, "[createAndChangeRepeatingRequest] mSession is null");
                    }
                }
            }
        } catch (CameraAccessException e) {
            LogHelper.e(TAG, "[createAndChangeRepeatingRequest] fail");
            e.printStackTrace();
        }
    }
    ...
    public void openCamera(@Nonnull ISettingManager settingManager, @Nonnull String cameraId,
        ...
        synchronized (CameraApplication.class) {
            ...
            CameraSysTrace.onEventSystrace("videoDevice.openCamera", true);
            ...
            CameraSysTrace.onEventSystrace("videoDevice.openCamera", false);
    ...
    public void takePicture(@Nonnull JpegCallback callback) {
        ...
        mVideoHandler.sendEmptyMessage(HANDLER_TAKE_VSS_CAPTURE);
        LogHelper.e(TAG, "[takePicture] -");
    }
    ...
    public void closeCamera(boolean sync) {
        synchronized (CameraApplication.class) {
            ...
            if (CameraState.CAMERA_UNKNOWN != mCameraState) {
                try {
                    mDeviceLock.tryLock(WAIT_TIME, TimeUnit.SECONDS);
                    mVideoHandler.removeMessages(HANDLER_TAKE_VSS_CAPTURE);
    ...
    public boolean isVssSupported(int cameraId) {
        ...
        if((!deviceDescription.isHDR10VssSupprot())&&(mSettingManager.getSettingController().queryValue(KEY_HDR10_PLUS))!=null){
    ...
    public void doCameraOpened(@Nonnull Camera2Proxy camera2proxy) {
            ...
            CameraSysTrace.onEventSystrace("videoDevice.doCameraOpened", true);
            try {
            ...
            CameraSysTrace.onEventSystrace("videoDevice.doCameraOpened", false);
    ...
    private void configureSession() {
        ...
        CameraSysTrace.onEventSystrace("videoDevice.configureSession", true);
        abortOldSession();
        ...
        try {
            ...
            if(getDynamicRangeProfile()!= 0) {
                List<OutputConfiguration> mOutputConfigs = new ArrayList<>();
                for (Surface surface : surfaces) {
                    if (surface != capture) {
                      OutputConfiguration out=new OutputConfiguration(surface);
                      out.setDynamicRangeProfile(getDynamicRangeProfile());
                      mOutputConfigs.add(out);
                      continue;
                    }

                    mOutputConfigs.add(new OutputConfiguration(surface));
                }
                LogHelper.d(TAG, "configureSession with mOutputConfigs = "+getDynamicRangeProfile());
                mCamera2Proxy.createCaptureSession(mSessionCallback,
                        mModeHandler, builder, mOutputConfigs);
            }else {
        ...
        CameraSysTrace.onEventSystrace("videoDevice.configureSession", false);
    ...
    private long getDynamicRangeProfile(){
        long profile= 0;
        String standardvalue=mSettingManager.getSettingController().queryValue("key_standard_hdr10");
        if(standardvalue!=null){
         profile = Long.parseLong(standardvalue);
        }

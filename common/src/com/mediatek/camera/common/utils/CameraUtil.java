...
public final class CameraUtil {
    ...
    private static int mInsetsHeight = 0;
    ...
    /**
    * Set the settingView icon state
    **/
    public static void setChangeIconState(@Nonnull boolean isChange){
        mChangeIconSate = isChange;
    }

    /**
    * get the settingView icon state
    **/
    ...
    /**
    * Set whether the settingView icon is displayable
    **/
    public static void setShowViewIcon(@Nonnull boolean isShow){
        mShowSettingIcon = isShow;
    }

    /**
    * get whether the settingView icon is displayable
    **/
    ...
    /**
     * check is GMS load or not
     */
    public static boolean checkGMSLoad() {

        String persistVersion = SystemProperties.getString("ro.com.google.gmsversion",
                "null");
        if (persistVersion.equals("null")) {
            return false;
        } else {
            return true;
        }
    ...
    public static android.media.ExifInterface getExif(byte[] jpeg) {
        android.media.ExifInterface exifInterface = null;
        try {
            exifInterface = new android.media.ExifInterface(new ByteArrayInputStream(jpeg));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static Size getSizeFromSdkExif(android.media.ExifInterface exifInterface) {
        int width = 0;
        int height = 0;
        width = exifInterface.getAttributeInt(android.media.ExifInterface.TAG_IMAGE_WIDTH, 0);
        height = exifInterface.getAttributeInt(android.media.ExifInterface.TAG_IMAGE_LENGTH, 0);
        LogHelper.d(TAG, "[getSizeFromSdkExif] from exifInterface,width = " + width + ",height = " + height);
        return new Size(width, height);
    }

    public static int getOrientationFromSdkExif(android.media.ExifInterface exifInterface) {
        int orientation = 0;
        Integer value = exifInterface.getAttributeInt(android.media.ExifInterface.TAG_ORIENTATION,
                0);
        switch (value.shortValue()) {
            case ExifInterface.Orientation.TOP_LEFT:
                orientation = 0;
    ...
    public static void rotateRotateLayoutChildView(Activity activity, View view, int orientation,
        ...
        LogHelper.d(TAG,"displayRotation="+displayRotation+",orientation="+orientation);
        if (displayRotation == 270 || displayRotation == 180) {
            if(SystemProperties.getInt("vendor.mtk.camera.app.platform.landscape", 0) == 0){
                orientation += 180;
            }
        }
        if (SystemProperties.getInt("vendor.mtk.camera.app.platform.landscape", 0) == 1) {
                orientation=resetGOrientation(orientation);
        }

        CameraUtil.rotateViewOrientation(view, orientation, animation);
    }

    private static int resetGOrientation(int orientation){
        if(orientation==0){
            orientation=270;
        } else {
            orientation-=90;
     ...
     */
    public static int getInsetsHeight() {
        return mInsetsHeight;
    }

    /**
     * set taskbar height.
     */
    public static void setInsetsHeight(int height) {
        mInsetsHeight = height;
    }
    ...
    /**
     * Judge if current device is aiv or not.
     * @return true: device is aiv.
     *         false: device is not aiv.
     */
    public static boolean isAivDevice() {
        boolean isAiv = "aiv"
                .equals(SystemProperties.getString("ro.build.characteristics", null));
        LogHelper.d(TAG, "isAIV_device = " + isAiv);
        return isAiv;
    }

    /**
     * Judge if current device is aiot or not.
     * @return true: device is aiot.
     *         false: device is not aiot.
     */
    public static boolean isAiotDevice() {
        boolean isAiot = "aiot"
                .equals(SystemProperties.getString("ro.build.characteristics", null));
        LogHelper.d(TAG, "isAIOT_device = " + isAiot);
        return isAiot;
    }
    ...
    public static void launchCamera(Activity activity) {
        ...
        FeatureLoader.updateSettingCurrentModeKey(activity.getApplicationContext(), defaultMode);
	...
	CameraApiHelper.getDeviceSpec(activity.getApplicationContext());
        ...
        try {
            deviceManagerFactory.getCameraDeviceManager(activity.getApplicationContext(), apiType).openCamera(
                    cameraId, null, null);
    ...
    /**
     * Get available front logical camera id.
     *
     * @return front logical camera id.
     */
    ...
    public static void initLogicalCameraId(DeviceDescription deviceDescription, String id) {
        ...
        if (mLogicalId == null || mDualZoomId == null || mFrontLogicalId == null) {
            if (deviceDescription.isStereoModeSupport()
                ...
                CameraCharacteristics characteristics =
                        deviceDescription.getCameraCharacteristics();
                if (characteristics != null) {
                    int facing = characteristics.get(CameraCharacteristics.LENS_FACING);
                    if (facing == CameraMetadata.LENS_FACING_FRONT) {
                        mFrontLogicalId = id;
                        LogHelper.d(TAG,
                                "<initLogicalCameraId> mFrontLogicalId is " + mFrontLogicalId);
                    } else {
                        mLogicalId = id;

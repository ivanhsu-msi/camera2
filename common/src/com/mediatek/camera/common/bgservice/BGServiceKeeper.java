...
public class BGServiceKeeper {
    ...
    public void releaseCaptureSurfaceList(){
        synchronized (mCaptureSurfaceList) {
            if (mCaptureSurfaceList.size() > 0) {
                mCaptureSurfaceList.remove(0);
            }
        }
    }

    public static boolean supportByBGService(String captureType) {
        LogHelper.d(TAG, "supportByBGService captureType = " + captureType);
        if (captureType == null) {
            return false;
        }
        return CAPTURE_TYPE_JPEG.equalsIgnoreCase(captureType);
    }
            ...
            synchronized (mCaptureSurfaceList) {
                ...
                if (mCaptureSurfaceList.isEmpty()) {
                    ...
                    if (status == REQUEST_SUCCESS) {
                        ...
                        captureSurface.notifyImageReader(imgReaderId);

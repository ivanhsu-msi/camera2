...
import com.mediatek.camera.feature.setting.zoommanualing.IZoomManualingViewListener;
...
public class CameraAppUI implements IAppUi {
    ...
    public void onCreate() {
        ...
        if (CameraUtil.isHasNavigationBar(mApp.getActivity())) {
            ...
            if (CameraUtil.isTablet()) {
               ...
               LogHelper.d(TAG, " onCreate displayRotation  " + displayRotation);
                ...
                if (com.mediatek.camera.portability.SystemProperties.getInt("vendor.mtk.camera.app.platform.landscape", 0) == 1) {
                    LogHelper.d(TAG, " in landscape case, enlarge the leftMargin with navigationBarHeight=" + navigationBarHeight);
                    params.leftMargin += navigationBarHeight;
                    appUI.setLayoutParams(params);
                }
            } else {
            ...
            if((CameraUtil.isTablet()||CameraUtil.isAivDevice() || CameraUtil.isAiotDevice())&& 
                                           Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU ) {
                // Note: Although the Taskbar doesn't have its own inset type,
                // its dimension can be retrieved using WindowInsetsCompat.Type.navigationBars()
                // or WindowInsetsCompat.Type.systemBars().
                ViewCompat.setOnApplyWindowInsetsListener(appUI, (v, windowInsets) -> {
                    ...
                    LogHelper.e(TAG,"insets.bottom="+insets.bottom+",params.bottomMargin="+params.bottomMargin);

                    CameraUtil.setInsetsHeight(insets.bottom);

                    int displayRotation = CameraUtil.getDisplayRotation(mApp.getActivity());
                    int InsetsHeight = CameraUtil.getInsetsHeight();
                    LogHelper.d(TAG, "displayRotation  " + displayRotation);
                    if (displayRotation == 90 || displayRotation == 270) {
                        params.leftMargin = InsetsHeight;
                        v.setLayoutParams(params);
                    } else {
                        params.bottomMargin = InsetsHeight;
                        v.setLayoutParams(params);
                    }

                    if (com.mediatek.camera.portability.SystemProperties.getInt("vendor.mtk.camera.app.platform.landscape", 0) == 1) {
                        LogHelper.d(TAG, " in landscape case, enlarge the leftMargin with InsetsHeight=" + InsetsHeight);
                        params.leftMargin = InsetsHeight;
                        v.setLayoutParams(params);
                    }

                    /*after OnApplyWindowInsetsListener callback call layoutSettingUI() again,
                    because CameraUtil.getInsetsHeight()=0 and the setting item will be overlap by taskbar before this callback */
                    layoutSettingUI();
    ...
    private void layoutSettingUI() {
        ...
        if (CameraUtil.isHasNavigationBar(mApp.getActivity())) {
            ...
            if((CameraUtil.isTablet()||CameraUtil.isAivDevice() || CameraUtil.isAiotDevice()) && 
                                            Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU ) {
                int InsetsHeight = CameraUtil.getInsetsHeight();
                LogHelper.d(TAG, " layoutSettingUI InsetsHeight  " + InsetsHeight);
                // set the setting layout don't contain taskbar height.

                LinearLayout settingContainer = (LinearLayout) settingRootView
                        .findViewById(R.id.setting_container);
                LinearLayout.LayoutParams containerParams =
                        (LinearLayout.LayoutParams) settingContainer.getLayoutParams();
                containerParams.bottomMargin = InsetsHeight;
                LogHelper.e(TAG,"containerParams="+containerParams);
                settingContainer.setLayoutParams(containerParams);

                LinearLayout settingTail = (LinearLayout) settingRootView
                        .findViewById(R.id.setting_tail);

                //set setting tail view height as taskbar height.
                LinearLayout.LayoutParams params =
                        (LinearLayout.LayoutParams) settingTail.getLayoutParams();
                params.height = InsetsHeight;
                settingTail.setLayoutParams(params);
    ...
    private class SettingStateListener implements SettingFragment.StateListener {
        ...
        public void onCreate() {
            ...
            if(!mCurrentModeName.equals(mApp.getActivity().getResources().getString(R.string.qrcode_mode_title))) {
	...
	**/
    ...
    private IZoomManualingViewListener.OnZMChangeViewListener mViewListener;

    @Override
    public ViewGroup getZMRootView() {
        return (ViewGroup) mApp.getActivity()
                .findViewById(R.id.zm_feature_root);
    }

    @Override
    public void updateZMView() {
        if(mViewListener != null){
            mViewListener.onZMChangeView();
        }
    }

    @Override
    public void addOnViewChangeListener(IZoomManualingViewListener.OnZMChangeViewListener
                                                    listener) {
        mViewListener = listener;
    }

    @Override
    public void removeOnViewChangeListener(IZoomManualingViewListener.OnZMChangeViewListener
                                                       listener) {
        listener = null;
        mViewListener = null;
    }

...
public class WaitDoneHandler extends Handler {
    ...
    private static final String TAG = WaitDoneHandler.class.getSimpleName();

    public WaitDoneHandler(Looper looper) {
        super(looper);
    }

    public <T> T blockRequest(final Callable<T> callable) {
        final Object[] call = new Object[]{null};
        post(new Runnable() {
            @Override
            public void run() {
                try {
                    call[0] = callable.call();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
    ...
    private boolean waitDone() {
        ...
        synchronized (waitDoneObject) {
            boolean postSuccess = post(unlockRunnable);
            if (postSuccess) {
                try {
                    waitDoneObject.wait();

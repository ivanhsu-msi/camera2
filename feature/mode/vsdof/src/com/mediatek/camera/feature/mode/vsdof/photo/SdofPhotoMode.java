...
import com.mediatek.camera.feature.setting.focus.FocusViewController;
...
public class SdofPhotoMode extends CameraModeBase implements CaptureDataCallback,
    ...
    private SdofPhotoGestureImpl mSdofPhotoGestureImpl = new SdofPhotoGestureImpl();
    ...
    public void pause(@Nullable DeviceUsage nextModeDeviceUsage) {
        ...
        if (mNeedCloseCameraIds.size() > 0) {
            ...
            mISdofDeviceController.closeSession();
        } else {
            clearAllCallbacks(mCameraId);
    ...
    public void unInit() {
        ...
        if (mCurrentFacing) {
            mIApp.getAppUi().unregisterGestureListener(mSdofPhotoGestureImpl);
        }
    }
    ...
    private void prepareAndOpenCamera(boolean needOpenCameraSync, String cameraId,
        ...
        statusMonitor.registerValueChangedListener(SELF_TIMER_STATE_KEY, mStatusChangeListener);
        ...
        mCurrentFacing = CameraUtil.isCameraFacingFront(mIApp.getActivity(), Integer.parseInt(mCameraId));
        if (mCurrentFacing) {
            mIApp.getAppUi().registerGestureListener(mSdofPhotoGestureImpl, 0);
        }
    }
    ...
    private void clearAllCallbacks(String cameraId) {
        ...
        statusMonitor.unregisterValueChangedListener(SELF_TIMER_STATE_KEY, mStatusChangeListener);
    }
    ...
    private void saveData(byte[] jpegData) {
        if (jpegData != null) {
            ...
            Size exifSize = CameraUtil.getSizeFromSdkExif(jpegData);
    ...
    private class MyStatusChangeListener implements StatusChangeListener {
        ...
        public void onStatusChanged(String key, String value) {
            ...
            } else if (SELF_TIMER_STATE_KEY.equals(key)) {
                if (PHOTO_CAPTURE_START.equals(value)) {
                    mSdofViewCtrl.setVisible(false);
                    mIsSelfTimerStart = true;
                } else if (PHOTO_CAPTURE_STOP.equals(value)) {
                    if (mIsSelfTimerStart) {
                        mSdofViewCtrl.setVisible(true);
    ...
    private SdofViewCtrl.ViewChangeListener mViewChangeListener
        ...
        public void onVsDofLevelChanged(int level) {
            if ((!mIsSelfTimerStart) && (mISdofDeviceController != null)) {
    ...
    private class SdofPhotoGestureImpl implements IAppUiListener.OnGestureListener {
        ...
        public boolean onSingleTapUp(float x, float y) {
            LogHelper.d(TAG, "[onSingleTapUp] x = " + x + " y = " + y);
            if (mCurrentFacing) {
                mISdofDeviceController.setVsDofTouchPositionParameter(x, y);
            }
            return false;
        }

...
import static com.mediatek.camera.feature.setting.dualcamerazoom.IDualZoomConfig.KEY_DUAL_ZOOM;
...
class SdofPhotoDeviceController extends Device2Controller implements
    ...
    private static final String DOF_TOUCH_POSITION_KEY = "com.mediatek.stereofeature.touchposition";
    ...
    private int[] CURRENT_DOF_TOUCH_POSITION = new int[]{(int) mCurrentX, (int) mCurrentY};
    ...
    private CaptureRequest.Key<int[]> mDofTouchPositionKey = null;
    ...
    /**
     * SdofPhotoDeviceController may use activity to get display rotation.
    ...
    public void openCamera(DeviceInfo info) {
        ...
        if (canOpenCamera(cameraId)) {
            try {
                ...
                mDofTouchPositionKey = CameraUtil.getRequestKey(
                        mCameraCharacteristics, DOF_TOUCH_POSITION_KEY);
    ...
    @Override
    public void setVsDofTouchPositionParameter(float x, float y) {
    ...
    private void setSpecialVendorTag(CaptureRequest.Builder builder) {
        ...
        if (mDofTouchPositionKey != null) {
            CURRENT_DOF_TOUCH_POSITION[0] = (int) mCurrentX;
            CURRENT_DOF_TOUCH_POSITION[1] = (int) mCurrentY;
            builder.set(mDofTouchPositionKey, CURRENT_DOF_TOUCH_POSITION);
            LogHelper.d(TAG, "[setSpecialVendorTag] dof touch position x = " + mCurrentX
                    + " & y = " + mCurrentY);
        }
    ...
    private void initSettings() throws CameraAccessException {
        ...
        sRelation = SdofPhotoRestriction.getRestriction()
                .getRelation("on", false);
        ...
        if (mCurrentCameraId != null){
            DeviceDescription deviceDescription = CameraApiHelper.getDeviceSpec(
                    ...
                    .getDeviceDescriptionMap()
                    .get(mCurrentCameraId);
            CameraCharacteristics.Key<int[]> zoomSets = deviceDescription.getKeyVsdofKeyOpticalZoomSets();
            if (zoomSets != null){
                sRelation.addBody(KEY_DUAL_ZOOM,"on","on");
            }
        }
    ...
    private final CaptureCallback mCaptureCallback = new CaptureCallback() {
        ...
        public void onCaptureFailed(@Nonnull CameraCaptureSession session,
            ...
            if (mCaptureFrameMap.containsKey(String.valueOf(failure.getFrameNumber()))){
                mModeDeviceCallback.onCaptureCallback();
            }
            if (mCamera2Proxy == null
                    || mModeDeviceCallback == null

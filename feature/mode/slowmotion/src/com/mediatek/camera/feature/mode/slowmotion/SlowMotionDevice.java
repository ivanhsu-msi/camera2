...
public class SlowMotionDevice extends Device2Controller
    ...
    private static final String SMVR_PREVIEW_RANGE = "com.mediatek.smvrfeature.smvrPreviewFps";
    private static final int HANDLER_UPDATE_PREVIEW_SURFACE = 1;
    private static final int STOP_BURST_RECORDING = 2;
    ...
    public void createAndChangeRepeatingRequest() {
        try {
            if (mCamera2Proxy == null || mCameraState != SlowMotionDevice.CameraState.CAMERA_OPENED) {
                LogHelper.e(TAG, "camera is closed or in opening state can't request ");
                return;
            }
            CaptureRequest.Builder builder =
                    doCreateAndConfigRequest(mIsRecording, false);
    ...
    public CaptureRequest.Builder createAndConfigRequest(int templateType) {
        ...
        try {
            builder = doCreateAndConfigRequest(mIsRecording, false);
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
        return builder;
    ...
    public void takePicture(@Nonnull JpegCallback callback) {
        ...
        try {
            ...
            LogHelper.i(TAG, "[takePicture] CONTROL_AE_TARGET_FPS_RANGE = "
                    + mSlowMotionSpec.fpsRangeForRecording);
            CaptureRequest.Key<int[]> key = mDeviceDescription.getKeySMVRV2RequestMode();
                if(key == null){
                    LogHelper.i(TAG, "[takePicture] not V2 ");
                    key = mDeviceDescription.getKeySMVRRequestMode();
                    builder.set(key,mSlowMotionSpec.meta.toV1RequestParams());
                    LogHelper.i(TAG, "requestKer is [" + key
                                + "]  [takePicture] set SMVR request mode = [" 
                                + mSlowMotionSpec.meta.toV1RequestParams()[0] + ", " 
                                + mSlowMotionSpec.meta.toV1RequestParams()[1] + "]");
                }else{
                    builder.set(key,mSlowMotionSpec.meta.toRequestParams());
                    LogHelper.i(TAG, "requestKer is [" + key
                                + "]  [takePicture] set SMVR request mode = [" 
                                + mSlowMotionSpec.meta.toRequestParams()[0] + ", " 
                                + mSlowMotionSpec.meta.toRequestParams()[1] + ", " 
                                + mSlowMotionSpec.meta.toRequestParams()[2] + "]");
            }
    ...
    @Override
    public void modePaused(boolean isPaused) {
        mIsPaused = isPaused;
    }
    ...
    private void setBurstRequest(List<CaptureRequest> requestList) {
        if (mSession != null) {
            synchronized (mSession) {
                if (mSession != null) {
                    try {
                        mIsBurstRecording = true;
                        mSession.captureBurst(requestList,
                                mPreviewCapProgressCallback, null);
                    } catch (CameraAccessException e) {
                        LogHelper.e(TAG, "[setRepeatingBurst] fail");
                        e.printStackTrace();
    ...
    /**
     * without value in SMVR_PREVIEW_RANGE
     * return default value 30fps
     */
    private int getPreviewRange(){
        CameraCharacteristics.Key<int[]> previewRange;
        List<CameraCharacteristics.Key<?>> keys = mCharacteristics.getKeys();
        for (CameraCharacteristics.Key<?> key : keys) {
            if (SMVR_PREVIEW_RANGE.equals(key.getName())) {
                previewRange = (CameraCharacteristics.Key<int[]>) key;
                int [] range = mCharacteristics.get(previewRange);
                if(range != null) {
                    LogHelper.i(TAG, "[getPreviewRange] Range = " + range[0]);
                    return range[0];
                }else {
                    LogHelper.i(TAG, "[getPreviewRange] Range = null");
                    break;
                }
            }
        }
    ...
    private void configureSession() {
        ...
        try {
            ...
            if (mSlowMotionSpec.pattern == SlowMotionSpec.Pattern.NORMAL) {
                ...
                CaptureRequest.Key<int[]> key = mDeviceDescription.getKeySMVRV2RequestMode();
                if(key == null){
                    LogHelper.i(TAG, "[configureSession] not V2 ");
                    key = mDeviceDescription.getKeySMVRRequestMode();
                    builder.set(key,mSlowMotionSpec.meta.toV1RequestParams());
                    LogHelper.i(TAG, "requestKer is ["+key 
                                +"]  [configureSession] set SMVR request mode = [" 
                                + mSlowMotionSpec.meta.toV1RequestParams()[0] + ", " 
                                + mSlowMotionSpec.meta.toV1RequestParams()[1] + "]");
                }else{
                    builder.set(key,mSlowMotionSpec.meta.toRequestParams());
            ...
            new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
                    ...
                    if (CameraState.CAMERA_OPENED == mCameraState
                        ...
                        if (mIsBurstRecording) {
                            CaptureResult.Key<int[]> key = mDeviceDescription.getKeySMVRV2BurstResult();
                            if(key == null){
                                key = mDeviceDescription.getKeySMVRBurstResult();
                            }
                            int[] burstResult = result.get(key);
                            if (burstResult == null || burstResult.length <= 0) {
                                LogHelper.i(TAG, "[burstResult] =  null");
                                return;
                            }
                            mBurstSum = mBurstSum + burstResult[0];
                            if (mBurstSum >0 && burstResult[0] == 0) {
                                LogHelper.i(TAG, "[burstResult] mBurstNum = " + mBurstSum);
                                mBurstSavedCallback.onBurstSaved();
                                mVideoHandler.sendEmptyMessage(STOP_BURST_RECORDING);
                                mBurstSum = 0;
                            }
                        }
                    }
        ...
        if (mCamera2Proxy != null) {
            ...
            if (mSlowMotionSpec.pattern != SlowMotionSpec.Pattern.NORMAL) {
                ...
                if(key == null){
                    ...
                    builder.set(key,mSlowMotionSpec.meta.toV1RequestParams());
                    LogHelper.i(TAG, "requestKer is ["+key
                                + "]  [doCreateAndConfigRequest] set SMVR request mode = [" 
                                + mSlowMotionSpec.meta.toV1RequestParams()[0] + ", " 
                                + mSlowMotionSpec.meta.toV1RequestParams()[1] + "]");
                }else{
                    builder.set(key,mSlowMotionSpec.meta.toRequestParams());
        ...
        synchronized (mPreparedSurfaces) {
            ...
            try {
                ...
                while (waitTimeRemaining > 0) {
                    ...
                    mPreparedSurfaces.wait(timeoutMs/10);
    ...
    public void burstRecording(BurstSavedCallback savedCallback) {
        ...
        try {
            ...
            for(int i = 0;i<getPreviewRange();i++){
                burstlist.add(builder.build());
            }
            setBurstRequest(burstlist);
            burstlist.clear();
        } catch (CameraAccessException e) {
    ...
    public void stopBurstRecording() {
        if(!mIsRecording){
            LogHelper.i(TAG, "[stopBurstRecording] can not call , recording stoped ");
            return;
        }

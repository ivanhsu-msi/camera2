...
class LongExposureDevice2Controller extends Device2Controller implements
    ...
    private static final int WAIT_DEVICE_OPENING_DONE_TIME = 80;
    ...
    public void closeCamera(boolean sync) {
        ...
        if(mCameraState == CameraState.CAMERA_OPENING){
            try{
                Thread.sleep(WAIT_DEVICE_OPENING_DONE_TIME);
                LogHelper.i(TAG, "[closeCamera] wait opening done , new state = " + mCameraState);
            } catch (InterruptedException e){
                e.printStackTrace();
            }
        }

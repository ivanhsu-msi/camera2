    ...
    public void onDataReceived(IVisualSearchDeviceController.DataCallbackInfo dataCallbackInfo) {
        ...
        if (data != null && format == ImageFormat.JPEG) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    LogHelper.d(TAG, "onDataReceived triggerShutterButtonClick");
                    saveData(data);
                    queryImage(data);
                }
            }).start();
            ...
            singleThreadExecutor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (data != null) {
                        ...
                        LogHelper.e(TAG, "yuvToBitmapPre run time: " + (endTime - startTime));
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_UPDATE_RECOGNITION:
                    if (mPreviewWidth > 0 && mPreviewHeight > 0 && CameraUtil.isChangeIconState() && mVisualSearchView != null) {

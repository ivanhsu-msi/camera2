...
abstract class ContinuousShotBase extends SettingBase implements
    ...
    protected static final String KEY_CSHOT_RESET_EV = "reset_ev";
    ...
    protected boolean stopContinuousShot() {
        mStatusResponder.statusChanged(KEY_CSHOT, KEY_CSHOT_RESET_EV);
        // send stop restriction
        return false;
    }

    protected abstract boolean isContinuousShotSupported();
    ...
    protected void postRestriction(String isOn) {
        ...
        if (!CONTINUOUSSHOT_ON.equals(isOn) && CameraUtil.getDualZoomId() != null) {
            LogHelper.i(TAG, "[postRestriction] CONTINUOUSSHOT off, set dualzoom on");
            baseRalation.addBody(KEY_DUAL_ZOOM, "on", "on, off");
        }
    ...
    protected void saveJpeg(byte[] data,int format) {
        ...
        if (format == ImageFormat.HEIC) {
            LogHelper.d(TAG, "[saveJpeg] HEIC get exif");
            exif = CameraUtil.getExif(data);
        }
        synchronized (mCSlock) {
            mCurrentShotsNum++;
        ...
        Size jpegSize;
        if (format == ImageFormat.HEIC) {
            jpegSize = CameraUtil.getSizeFromSdkExif(exif);
        } else {
            jpegSize = CameraUtil.getSizeFromSdkExif(data);
        }
        ContentValues contentValues;
        if (format == ImageFormat.HEIC) {
            contentValues = mCsNamingRule.createContentValues(exif,format, mFileDirectory,
                    jpegSize.getWidth(), jpegSize.getHeight(), mShutterTime, mCurrentShotsNum);
        } else {
            contentValues = mCsNamingRule.createContentValues(data,format, mFileDirectory,
                    jpegSize.getWidth(), jpegSize.getHeight(), mShutterTime, mCurrentShotsNum);
        }
    ...
    private void updateThumbnail(byte[] bytes, android.media.ExifInterface exif) {
        LogHelper.d(TAG, "updateThumbnail() exif");
        Bitmap bitmap = BitmapCreator.createBitmapFromJpeg(bytes, mApp.getAppUi()
                .getThumbnailViewWidth(), exif);
        mApp.getAppUi().updateThumbnail(bitmap);
    }

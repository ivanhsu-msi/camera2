...
class CsNamingRule {
    ...
    private static final String FILE_FORMAT_SUFFIX_JPG = ".jpg";
    private static final String FILE_FORMAT_SUFFIX_HEIC = ".heic";
     ...
     */
    ContentValues createContentValues(byte[] data,int format, String fileDirectory, int
        ...
        int orientation = CameraUtil.getOrientationFromSdkExif(data);

        String mime = format == ImageFormat.HEIC? "image/heic":"image/jpeg";
        String suffix = format==ImageFormat.HEIC? FILE_FORMAT_SUFFIX_HEIC:FILE_FORMAT_SUFFIX_JPG;
        String fileName = mImageFileName.generateTitle(shutterTime) +
                FILE_NAME_CONNECTOR + count + FILE_NAME_SUFFIX + suffix;
    ...
    /**
     * create a content values from exif.
     *
     * @param exif  the exif of data, avoid new android.media.ExifInterface object repeatly.
     * @param format the format of the data
     * @param fileDirectory file directory.
     * @param pictureWidth  the width of content values.
     * @param pictureHeight the height of content values.
     ...
     * @return the content values from the data.
     */
    ContentValues createContentValues(android.media.ExifInterface exif,int format, String fileDirectory, int
            pictureWidth, int pictureHeight, long shutterTime, int count) {
        ContentValues values = new ContentValues();
        String title = mImageFileName.generateTitle(shutterTime) +
                FILE_NAME_CONNECTOR + count + FILE_NAME_SUFFIX;
        int orientation = CameraUtil.getOrientationFromSdkExif(exif);

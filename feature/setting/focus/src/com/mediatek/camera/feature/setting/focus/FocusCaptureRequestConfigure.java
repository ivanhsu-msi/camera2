...
public class FocusCaptureRequestConfigure implements ICameraSetting.ICaptureRequestConfigure, IFocus
    ...
    private static final long AE_AF_TIME_OUT_MILLIONS = 5 * 1000;
    ...
    private boolean mNeedWaitTafDone = false;
    ...
    private int[] mAfRegion = {0, 0, 0, 0};
    private int[] mAeRegion = {0, 0, 0, 0};
    private boolean mResetAFRoi = false;
    private boolean mResetAERoi = false;
    ...
    private static final int DEFAULT_RECT_VALUE = 0;
    ...
    private CaptureRequest.Key<int[]> mKeyAfRoiRequest;
    private CaptureRequest.Key<int[]> mKeyAeRoiRequest;
    ...
    private volatile boolean mAeTriggerStarted;
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        mKeyFlashCalibrationState = deviceDescription.getKeyFlashCalibrationState();
    ...
    private boolean checkRectValue(List<Camera.Area> area, int[] region) {
        region[0] = area.get(0).rect.left;
        region[1] = area.get(0).rect.top;
        region[2] = area.get(0).rect.right;
        region[3] = area.get(0).rect.bottom;

        if (area.get(0).rect.left < 0 || area.get(0).rect.top < 0 ||
                area.get(0).rect.right < 0 || area.get(0).rect.bottom < 0) {
            area.get(0).rect.left = DEFAULT_RECT_VALUE;
            area.get(0).rect.top = DEFAULT_RECT_VALUE;
            area.get(0).rect.right = DEFAULT_RECT_VALUE;
            area.get(0).rect.bottom = DEFAULT_RECT_VALUE;
            return true;
        }
        return false ;
    }
    ...
    private void sendAutoFocusTriggerCaptureRequest(boolean needCancelAutoFocus) {
        LogHelper.d(TAG, "[sendAutoFocusTriggerCaptureRequest] needCancelAutoFocus "
                + needCancelAutoFocus + ",mNeedWaitTafDone = "+mNeedWaitTafDone);
        ...
        LogHelper.d(TAG, "[sendAutoFocusTriggerCaptureRequest] mKeyAfRoiRequest = "
                + mKeyAfRoiRequest + "mKeyAeRoiRequest = "
                + mKeyAeRoiRequest);
        if (mResetAFRoi && mKeyAfRoiRequest != null) {
            builder.set(mKeyAfRoiRequest, mAfRegion);
            LogHelper.d(TAG, "[sendAutoFocusTriggerCaptureRequest] mKeyAfRoiRequest"
                    + mKeyAfRoiRequest
                    + "mAFRegion : "
                    + "[" + mAfRegion[0] + "," + mAfRegion[1] + ','
                    + mAfRegion[2] + "," + mAfRegion[3] + "]");
            mResetAFRoi = false;
        }

        if (mResetAERoi && mKeyAeRoiRequest != null) {
            builder.set(mKeyAeRoiRequest, mAeRegion);
            LogHelper.d(TAG, "[sendAutoFocusTriggerCaptureRequest] mKeyAeRoiRequest"
                    + mKeyAeRoiRequest
                    + "mAERegion : "
                    + "[" + mAeRegion[0] + "," + mAeRegion[1] + ','
                    + mAeRegion[2] + "," + mAeRegion[3] + "]");
            mResetAERoi = false;
        }
    ...
    private void showFlashCalibrationResult(CaptureResult result) {
    ...
    private void initFlashCalibrationVendorKey(CameraCharacteristics cs) {
        ...
        if (deviceDescription != null) {
            ...
            mKeyAfRoiRequest = deviceDescription.getKeyMultiCamAfRoi();
            mKeyAeRoiRequest = deviceDescription.getKeyMultiCamAeRoi();
            LogHelper.d(TAG, "[initFlashCalibrationVendorKey] mIsFlashCalibrationSupported "
                    ...
                    + mKeyFlashCalibrationResult + "mKeyAfRoiRequest"
                    + mKeyAfRoiRequest + "mKeyAeRoiRequest"
                    + mKeyAeRoiRequest);
        }
    }
    ...
    private void autofocusStateChangeDispatcher(CaptureResult result,boolean isTimeout) {
        ...
        if (isTimeout){
            resultAFState = CaptureRequest.CONTROL_AF_STATE_FOCUSED_LOCKED;
            mAeTriggerStarted = true;
            mAeState = CaptureRequest.CONTROL_AE_STATE_CONVERGED;
        }
                        ...
                        if (mAeState == CaptureResult.CONTROL_AE_STATE_CONVERGED
                            ...
                            if (mLastResultAFState != afState){
                                ...
                                LogHelper.d(TAG, "[notifyFocusStateChanged] mLastResultAFState " +
                                        ...
                                        +"autoFocus time " + (System.currentTimeMillis()-mStartTime));
                            ...
                            LogHelper.d(TAG, "[notifyFocusStateChanged] ae triggered but not " +
                                    "converged");
                        }
                    }else{
                        if (mLastResultAFState != afState){
    ...
    private CameraCaptureSession.CaptureCallback mPreviewCallback
        ...
        public void onCaptureStarted(CameraCaptureSession session, CaptureRequest request,
            ...
            if (CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER_START
                ...
                LogHelper.i(TAG, "[onCaptureStarted] ae precapture triggered");
                mAeTriggerStarted = true;
            }
        }
        ...
        public void onCaptureProgressed(CameraCaptureSession session, CaptureRequest request,
            ...
            if (CameraUtil.isStillCaptureTemplate(request)
                    && P2DoneInfo.checkP2DoneResult(result)) {
                synchronized (mFocusQueue) {
                    LogHelper.d(TAG, "[onCaptureProgressed] picture done");
                    if (!mFocusQueue.isEmpty() && CANCEL_AUTOFOCUS.equals(mFocusQueue.peek())) {
                        LogHelper.d(TAG, "[onCaptureProgressed] mFocusQueue " + mFocusQueue.size() +
                                " do cancelAutoFocus");
                        mFocusQueue.clear();
                        cancelAutoFocus();
                        mFocus.resetTouchFocusWhenCaptureDone();
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
            ...
            boolean timeOut = mStartTime != 0
                    && mFocusQueue.contains(AUTOFOCUS)
                    && (System.currentTimeMillis() - mStartTime > AE_AF_TIME_OUT_MILLIONS);
            if (!isFlashCalibrationProcessing(result) && timeOut){
                LogHelper.e(TAG, "[onCaptureCompleted] time out go to capture immediately"
                        +" aeState = " + result.get(TotalCaptureResult.CONTROL_AE_STATE)
                        +" afState = " + result.get(CaptureResult.CONTROL_AF_STATE));
                autofocusStateChangeDispatcher(result,true);
            }else{
            ...
            if (!P2DoneInfo.checkP2DoneResult(result)) {
                synchronized (mFocusQueue) {
                    LogHelper.d(TAG, "[onCaptureProgressed] picture done");
                    if (!mFocusQueue.isEmpty() && CANCEL_AUTOFOCUS.equals(mFocusQueue.peek())) {
                        LogHelper.d(TAG, "[onCaptureProgressed] mFocusQueue " + mFocusQueue.size() +
    ...
    private boolean isFlashCalibrationProcessing(TotalCaptureResult result){
        //0 not running ,1 running
        final int FLASH_CALIBRATION_RUNNING = 1;
        if (mKeyFlashCalibrationState != null && result != null){
            int[] state=result.get(mKeyFlashCalibrationState);
            if (state != null && state.length == 1){
                return state[0] == FLASH_CALIBRATION_RUNNING;
            }else {
                LogHelper.w(TAG, "[isFlashCalibrationProcessing] state value is " + state);
            }
        }

        //intent set up flash_calibration on
        if (mKeyFlashCalibrationRequest != null){
            LogHelper.d(TAG, "[isFlashCalibrationProcessing] processing from activity intent");
            return true;
        }

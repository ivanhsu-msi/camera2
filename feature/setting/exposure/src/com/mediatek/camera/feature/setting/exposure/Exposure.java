...
public class Exposure extends SettingBase implements ExposureView.ExposureViewChangedListener,
    ...
    private static final String VALUE_CSHOT_RESET_EV = "reset_ev";
    ...
    private float mPrevBright;
    private Boolean mShutterButtonClick = false;
    ...
    public void init(IApp app, ICameraContext cameraContext, ISettingManager.SettingController
        ...
        mPrevBright =
                IWindowManagerExt
                        ...
                        .getAnimationScale(ANIMATOR_DURATION_SCALE_SELECTOR);
                LogHelper.d(mTag, "[init] - mCompensationOrientation " + mCompensationOrientation +
                   "mCurrESSLEDMinStep = " + mCurrESSLEDMinStep +
                   "mCurrESSOLEDMinStep = " + mCurrESSOLEDMinStep);

        Intent intent = mActivity.getIntent();
        if (intent != null && intent.getExtra("flash_calibration")!=null){
            mIsFlashCalibrationEnable = (boolean)intent.getExtra("flash_calibration");
        }
    }
    ...
    public boolean onShutterButtonClick() {
        ...
        if (mExposureListener != null && mExposureListener.isStandardFlowPanelFlashCaptureProcessing()){
            LogHelper.w(mTag, "[onShutterButtonClick] ignore, as last standard panel flash on going");
            return true;
        }
        ...
        if (mShutterButtonClick) {
            LogHelper.d(mTag, "[onShutterButtonClick] ignored as last capture still ongoing");
            return true;
        }
        mShutterButtonClick = true;
        if (mExposureListener != null && mExposureListener.checkTodoCapturAfterAeConverted()) {
            ...
            // no need wait AE converted and trigger shutter button directly.
            mShutterButtonClick = false;
            LogHelper.d(mTag, "[onShutterButtonClick] checkTodoCapturAfterAeConverted  = false");
            return false;
        }
    }
    ...
    protected String getCurrentZsdValue() {
        String zsdValue = mSettingController.queryValue(ZSD_KEY);
        LogHelper.d(mTag, "[getCurrentZsdValue] zsdValue : " + zsdValue);
        return zsdValue;
    }
    ...
    protected void updatePanelColor(int color){
        mAppUi.updatePanelColor(color);
    }
        ...
        public void onStatusChanged(String key, String value) {
            ...
            switch (key) {
                ...
                case VIDEO_STATUS_KEY:
                    if (mSettingChangeRequester == null){
                        LogHelper.w(mTag, "[onStatusChanged] mSettingChangeRequester is null");
                        break;
                    }
                ...
                case CSHOT_KEY:
                    ...
                    } else if (VALUE_CSHOT_RESET_EV.equals(value)) {
                        if (mExposureViewController != null && !hasDisableEvReset()
                                && getValue() != null && !getValue().equals("0")) {
                            mExposureViewController.resetExposureView();
                        }
    ...
    public boolean isFlashCalibrationEnable() {
        return mIsFlashCalibrationEnable;
    }

    public void postRestrictionBeforeDoAePreCapture() {
        if ("on".equalsIgnoreCase(getCurrentZsdValue())) {
            mSettingController.postRestriction(ExposureRestriction.getRestriction()
                    .getRelation(ExposureRestriction.EXPOSURE_PRE_CAPTURE_START, true));
    ...
    /**
     * notify other settings that ae pre capture trigger has started
     */
    public void notifyAePreCaptureStarted() {
        mStatusChangeListener.onStatusChanged(EXPOSURE_KEY,"pre_capture_started");
    }
}

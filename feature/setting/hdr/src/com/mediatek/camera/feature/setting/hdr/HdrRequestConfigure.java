...
public class HdrRequestConfigure implements ICameraSetting.ICaptureRequestConfigure, IHdr.Listener {
    ...
    private static final String HDR_KEY_STAGGER_HDR_AVAILABLE_MODE =
    ...
    private static final int MTK_HDR_FEATURE_HDR_HAL_MODE_STAGGER_2EXP = 8;
    private static final int MTK_HDR_FEATURE_HDR_HAL_MODE_STAGGER_3EXP = 16;
    ...
    private String mCurrentModeDeviceState = ICameraMode.MODE_DEVICE_STATE_UNKNOWN;
        ...
        */
        ...
        if (mKeyMSHdrAvailableMode != null) {
            ...
            if (smhdrModes != null && smhdrModes.length >= 1){
                for (int i:smhdrModes) {
                    if (i == MTK_HDR_FEATURE_HDR_HAL_MODE_MSTREAM_CAPTURE ||
                            i == MTK_HDR_FEATURE_HDR_HAL_MODE_MSTREAM_CAPTURE_PREVIEW) {
        ...
        if(mKeyHdrAvailableStaggerModes != null){
            int[] staggerhdrModes = characteristics.get(mKeyHdrAvailableStaggerModes);
            if (staggerhdrModes != null && staggerhdrModes.length >= 1){
                for (int i:staggerhdrModes) {
                    if(i == MTK_HDR_FEATURE_HDR_HAL_MODE_STAGGER_2EXP||
                            i == MTK_HDR_FEATURE_HDR_HAL_MODE_STAGGER_3EXP){
                        mIsMStreamHdrSupported = true;
    ...
    public void onHdrValueChanged() {
        if (ICameraMode.MODE_DEVICE_STATE_PREVIEWING.equals(mCurrentModeDeviceState)
                ||ICameraMode.MODE_DEVICE_STATE_OPENED.equals(mCurrentModeDeviceState)){
            LogHelper.w(TAG, "[onHdrValueChanged] mCurrentModeDeviceState " + mCurrentModeDeviceState);
            mSettingDevice2Requester.requestRestartSession();
        }else {
            LogHelper.w(TAG, "[onHdrValueChanged] hdr value changed but not configured." );
        }
    }

    @Override
    public void updateModeDeviceState(String newState) {
        if (!newState.equals(mCurrentModeDeviceState)) {
            mCurrentModeDeviceState = newState;
            LogHelper.d(TAG, "[updateModeDeviceState] mCurrentModeDeviceState " + mCurrentModeDeviceState);
        }
    }
    ...
    private void initHdrVendorKey(CameraCharacteristics cs) {
        ...
        if (deviceDescription != null) {
            ...
            mKeyHdrAvailableStaggerModes = deviceDescription.getKeyStaggerHDRAvailableModes();
        ...
        for (CameraCharacteristics.Key<?> key : keyList) {
            ...
            }else if (key.getName().equals(HDR_KEY_STAGGER_HDR_AVAILABLE_MODE)) {
                mKeyHdrAvailableStaggerModes = (CameraCharacteristics.Key<int[]>) key;
            }

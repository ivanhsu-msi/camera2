...
/**
 * configure postview info for thumbnail helper.
 */
...
public class PostViewCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    public PostViewCaptureRequestConfig(PostView pv,
                Context context, ISettingManager.SettingController controller) {
        mPv = pv;
        mContext = context;
        mSettingController = controller;
    }

    @Override
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        DeviceDescription deviceDescription = CameraApiHelper.getDeviceSpec(mContext)
                .getDeviceDescriptionMap().get(String.valueOf(mSettingController.getCameraId()));
        boolean isSupported = false;
        if (deviceDescription != null) {
            isSupported = deviceDescription.isThumbnailPostViewSupport();
        }
        mPv.initSupportValue(isSupported);
    }

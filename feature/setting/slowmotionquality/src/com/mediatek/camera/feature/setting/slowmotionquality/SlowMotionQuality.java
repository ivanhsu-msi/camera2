...
public class SlowMotionQuality extends SettingBase
    ...
    public void updateValue(String defaultValue, int mQualitiesSize) {
        String value = mDataStore.getValue(getKey(), defaultValue, getStoreScope());
        if (Integer.valueOf(value) >= mQualitiesSize){
            LogHelper.d(TAG,"updateValue value = " + value + " ,reset value = " + defaultValue);
            value = defaultValue;
        }
        setValue(value);

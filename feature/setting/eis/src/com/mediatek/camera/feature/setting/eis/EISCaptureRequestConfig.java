...
public class EISCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    public boolean isSupportHfps(String value){
        if (getEISMaxSizeInHfps() == null) {
            LogHelper.d(TAG, "is EIS SupportQuality no limit");
            return true;
        } else if (compareSize(currentVideoSize(value), getEISMaxSizeInHfps())) {
    ...
    private int [] currentVideoSize(String currentValue) {
        ...
        CamcorderProfile cm = CamcorderProfile.get(mEis.getCameraId(),Integer.parseInt(currentValue));
    ...
    /**
     * eis on ---> get max fps
     * eis on ---> get max videoQuality
     * @return
     */
    private int[] getEISMaxSizeInHfps() {
        DeviceDescription deviceDescription = CameraApiHelper.getDeviceSpec(mContext)
                .getDeviceDescriptionMap().get(String.valueOf(mEis.getCameraId()));
        if (deviceDescription != null &&

...
public class DngCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure,
    ...
    @Override
    public void notifyOverrideValue(boolean isOn,String modeKey) {
        LogHelper.i(TAG, "[notifyOverrideValue] ");
        if (isOn && mRawCaptureSurface == null
                && "com.mediatek.camera.common.mode.photo.PhotoMode".equalsIgnoreCase(modeKey)){
    ...
    private CameraCaptureSession.CaptureCallback mCaptureCallback
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request,
            ...
            if (mIsDngOn
                ...
                LogHelper.i(TAG, "onCaptureCompleted frame num " + result.getFrameNumber());
                mCaptureResult = result;
                mCaptureResultReady = true;
                mDngValueListener.onDngCreatorStateUpdate(true);
                convertRawToDng(mRawImage);
            } else if (!mIsDngOn && mRawDataReady && CameraUtil.isStillCaptureTemplate(result)) {
                LogHelper.i(TAG, "onCaptureCompleted Dng off and mRawDataReady is ready");
                mCaptureResult = result;
                mCaptureResultReady = true;
                mDngValueListener.onDngCreatorStateUpdate(true);
                convertRawToDng(mRawImage);

...
public class SelfTimerSoundManager {
    ...
    public void load() {
        if (mSoundPool == null) {
            ...
            LogHelper.d(TAG, "[load] mBeepOnce = "+mBeepOnce
                    +"mBeepTwice = " +mBeepTwice);
        }
    ...
    public void play(int soundId) {
        synchronized (this) {
            ...
            LogHelper.d(TAG, "[play] soundId = " + soundId
                    + " mBeepOnce = " + mBeepOnce + " mBeepTwice = " + mBeepTwice);
            if (soundId == BEEP_ONCE) {
                mBeepOnceStreamId = mSoundPool.play(mBeepOnce, 1.0f, 1.0f, 0, 0, 1.0f);
            } else if (soundId == BEEP_TWICE) {
                mBeepTwiceStreamId = mSoundPool.play(mBeepTwice, 1.0f, 1.0f, 0, 0, 1.0f);
            } else {
                LogHelper.e(TAG, "[play] illegal sound id == " + soundId);
            }
        }
    }

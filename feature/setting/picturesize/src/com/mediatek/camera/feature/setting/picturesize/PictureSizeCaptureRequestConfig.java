...
public class PictureSizeCaptureRequestConfig implements ICameraSetting.ICaptureRequestConfigure {
    ...
    /** This property is used to display all picture sizes and select the specified picture size*/
    private static final String TAG_PICTURE_SIZE_FORCE_SELECT = "vendor.debug.camera.set.jpeg.size";
    ...
    public void setCameraCharacteristics(CameraCharacteristics characteristics) {
        ...
        List<Size> yUVSupportedSize = getSupportedPictureSize(s, ImageFormat.YUV_420_888);
        sortSizeInDescending(supportedSizes);
        sortSizeInDescending(yUVSupportedSize);

        List<String> supportedSizesInStr = sizeToStr(supportedSizes);
        List<String> yUVSupportedSizesInStr = sizeToStr(yUVSupportedSize);
        mPictureSize.setYUVSupportSize(yUVSupportedSizesInStr);
        String forcePictureSize = SystemProperties.getString(TAG_PICTURE_SIZE_FORCE_SELECT,"");
        if (!TextUtils.isEmpty(forcePictureSize)){
            //display all picture size for hal to debug and select specified picture size
            mPictureSize.onValueInitialized(supportedSizesInStr,forcePictureSize);
        }else{
